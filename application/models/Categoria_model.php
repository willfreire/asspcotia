<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Categoria_model extends CI_Model {

    # Propriedades
    public $draw;
    public $orderBy;
    public $orderType;
    public $start;
    public $length;
    public $filter;
    public $columns;
    public $recordsTotal;
    public $recordsFiltered;

    public function __construct()
    {
        parent::__construct();
    }

    /**
     * Método responsável por cadastrar / editar uma categoria
     *
     * @method setCategoria
     * @param obj $valores Dados para cadastro / edicao
     * @access public
     * @return obj Status de ação
     */
    public function setCategoria($valores)
    {
        # Atribuir vars
        $retorno = new stdClass();
        $dados   = array();

        $dados['categoria'] = $valores->categoria;

        if (isset($valores->id) && $valores->id != ""):
            # Atualiza categoria
            $this->db->where('id_categoria_pk', $valores->id);
            $this->db->update('tb_categoria', $dados);

            if ($this->db->affected_rows() >= 0) {
                $retorno->status = TRUE;
                $retorno->msg    = "Edi&ccedil;&atilde;o realizada com Sucesso!";
            } else {
                $retorno->status = FALSE;
                $retorno->msg    = "Houve um erro ao editar! Tente novamente...";
            }
        else:
            # Grava categoria
            $this->db->insert('tb_categoria', $dados);

            if ($this->db->affected_rows() > 0) {
                $retorno->status = TRUE;
                $retorno->msg    = "Cadastro realizado com Sucesso!";
            } else {
                $retorno->status = FALSE;
                $retorno->msg    = "Houve um erro ao cadastrar! Tente novamente...";
            }
        endif;

        # retornar
        return $retorno;
    }

    /**
     * Método responsável por pesquisar e buscar categorias
     *
     * @method getCategorias
     * @param obj $search Conjuntos de dados para realizar a pesquisa
     * @access public
     * @return obj Lista de categorias
     */
    public function getCategorias($search)
    {
        # Atribuir valores
        $this->draw      = $search->draw;
        $this->orderBy   = $search->orderBy;
        $this->orderType = $search->orderType;
        $this->start     = $search->start;
        $this->length    = $search->length;
        $this->filter    = $search->filter;
        $this->columns   = $search->columns;
        $filter          = array();
        $where           = array();

        # Se houver busca pela grid
        if ($this->filter != NULL):
            for($i=0; $i<count($this->columns); $i++):
                if ($this->columns[$i]['searchable'] === "true"):
                    $column = $this->columns[$i]['data'];
                    $filter[]= "$column LIKE '%{$this->filter}%'";
                endif;
            endfor;
        endif;

        # Contar total de registros
        $this->db->select('COUNT(id_categoria_pk) AS total');
        $this->db->from('tb_categoria');
        if (!empty($filter)):
            $where = implode(" OR ", $filter);
            $this->db->where($where);
        endif;
        $query            = $this->db->get();
        $respRecordsTotal = $query->result();
        if (!empty($respRecordsTotal)):
            $this->recordsTotal = $respRecordsTotal[0]->total;
        else:
            $this->recordsTotal = 0;
        endif;

        # Consultar categorias
        $this->db->select('id_categoria_pk, categoria');
        $this->db->from('tb_categoria');
        if (!empty($filter)):
            $where = implode(" OR ", $filter);
            $this->db->where($where);
        endif;
        $this->db->order_by($this->orderBy, $this->orderType);
        $this->db->limit($this->length, $this->start);
        $query_dados = $this->db->get();
        $resp_dados  = $query_dados->result();

        # Criar classe predefinida
        $categs = array();
        if (!empty($resp_dados)):

            foreach ($resp_dados as $value):
                # Botao
                $id_categoria = $value->id_categoria_pk;
                $url_edit     = base_url('./admin/categoria/editar/'.$id_categoria);
                $url_view     = base_url('./admin/categoria/ver/'.$id_categoria);
                $acao         = "<button type='button' class='btn btn-success btn-xs btn-acao' title='Editar Categoria' onclick='Categoria.redirect(\"$url_edit\")'><i class='glyphicon glyphicon-edit' aria-hidden='true'></i></button>";
                $acao        .= "<button type='button' class='btn btn-warning btn-xs btn-acao' title='Visualizar Categoria' onclick='Categoria.redirect(\"$url_view\")'><i class='glyphicon glyphicon-eye-open' aria-hidden='true'></i></button>";
                $acao        .= "<button type='button' class='btn btn-danger btn-xs btn-acao' title='Excluir Categoria' onclick='Categoria.del(\"$id_categoria\")'><i class='glyphicon glyphicon-remove' aria-hidden='true'></i></button>";

                $categ            = new stdClass();
                $categ->categoria = $value->categoria;
                $categ->acao      = $acao;
                $categs[]         = $categ;
            endforeach;

        endif;

        $dados['draw']            = intval($this->draw);
        $dados['recordsTotal']    = $this->recordsTotal;
        $dados['recordsFiltered'] = $this->recordsTotal;
        $dados['data']            = $categs;

        return $dados;
    }

    /**
     * Método de exclusão de um Categoria
     *
     * @method delCategoria
     * @access public
     * @param integer $id Id do registro a ser excluído
     * @return obj Status da ação
     */
    public function delCategoria($id)
    {
        # Atribuir vars
        $retorno = new stdClass();
        $log     = array();

        # SQL
        $this->db->where('id_categoria_pk', $id);
        $this->db->delete('tb_categoria');

        if ($this->db->affected_rows() > 0) {
            $retorno->status = TRUE;
            $retorno->msg    = "Exclus&atilde;o realizada com Sucesso!";
        } else {
            $retorno->status = FALSE;
            $retorno->msg    = "Houve um erro ao Excluir! Tente novamente...";
        }

        # retornar
        return $retorno;
    }
}

/* End of file categoria_model.php */
/* Location: ./application/models/categoria_model.php */