<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Noticia_model extends CI_Model {

    # Propriedades
    public $draw;
    public $orderBy;
    public $orderType;
    public $start;
    public $length;
    public $filter;
    public $columns;
    public $recordsTotal;
    public $recordsFiltered;

    public function __construct()
    {
        parent::__construct();
    }

    /**
     * Método responsável por cadastrar / editar uma noticia
     *
     * @method setNoticia
     * @param obj $valores Dados para cadastro / edicao
     * @access public
     * @return obj Status de ação
     */
    public function setNoticia($valores)
    {
        # Atribuir vars
        $retorno = new stdClass();
        $dados   = array();
        $log     = array();
        $data    = date("Y-m-d H:i");

        $dados['noticia_url']  = convert_accented_characters(url_title($valores->titulo, "dash", TRUE));
        $dados['titulo']       = $valores->titulo;
        $dados['img']          = $valores->img;
        $dados['noticia']      = $valores->noticia;
        $dados['id_status_fk'] = $valores->status;
        $dados['dt_cadastro']  = $data;

        if (isset($valores->id) && $valores->id != ""):
            # Atualiza noticia
            $this->db->where('id_noticia_pk', $valores->id);
            $this->db->update('tb_noticia', $dados);

            if ($this->db->affected_rows() >= 0) {
                $retorno->status = TRUE;
                $retorno->msg    = "Edi&ccedil;&atilde;o realizada com Sucesso!";
                # Gravar Log
                $log['id_acao_pk']    = 2;
                $log['id_usuario_fk'] = $this->session->userdata('id_user');
                $log['id_noticia_fk'] = $valores->id;
                $log['dt_hr_cad']     = $data;
                $this->db->insert('tb_noticia_log', $log);
            } else {
                $retorno->status = FALSE;
                $retorno->msg    = "Houve um erro ao editar! Tente novamente...";
            }
        else:
            # Grava noticia
            $this->db->insert('tb_noticia', $dados);

            if ($this->db->affected_rows() > 0) {
                $retorno->status = TRUE;
                $retorno->msg    = "Cadastro realizado com Sucesso!";
                # Gravar Log
                $log['id_acao_pk']    = 1;
                $log['id_usuario_fk'] = $this->session->userdata('id_user');
                $log['id_noticia_fk'] = $this->db->insert_id();
                $log['dt_hr_cad']     = $data;
                $this->db->insert('tb_noticia_log', $log);
            } else {
                $retorno->status = FALSE;
                $retorno->msg    = "Houve um erro ao cadastrar! Tente novamente...";
            }
        endif;

        # retornar
        return $retorno;
    }

    /**
     * Método responsável por pesquisar e buscar noticias
     *
     * @method getNoticias
     * @param obj $search Conjuntos de dados para realizar a pesquisa
     * @access public
     * @return obj Lista de noticias
     */
    public function getNoticias($search)
    {
        # Atribuir valores
        $this->draw      = $search->draw;
        $this->orderBy   = $search->orderBy;
        $this->orderType = $search->orderType;
        $this->start     = $search->start;
        $this->length    = $search->length;
        $this->filter    = $search->filter;
        $this->columns   = $search->columns;
        $filter          = array();
        $where           = array();

        # Se houver busca pela grid
        if ($this->filter != NULL):
            for($i=0; $i<count($this->columns); $i++):
                if ($this->columns[$i]['searchable'] === "true"):
                    $column = $this->columns[$i]['data'];
                    $filter[]= "$column LIKE '%{$this->filter}%'";
                endif;
            endfor;
        endif;

        # Contar total de registros
        $this->db->select('COUNT(n.id_noticia_pk) AS total');
        $this->db->from('tb_noticia n');
        $this->db->join('tb_status s', 's.id_status_pk = n.id_status_fk', 'inner');
        if (!empty($filter)):
            $where = implode(" OR ", $filter);
            $this->db->where($where);
        endif;
        $query            = $this->db->get();
        $respRecordsTotal = $query->result();
        if (!empty($respRecordsTotal)):
            $this->recordsTotal = $respRecordsTotal[0]->total;
        else:
            $this->recordsTotal = 0;
        endif;

        # Consultar noticias
        $this->db->select('n.id_noticia_pk, n.noticia_url, n.titulo, n.img, n.noticia, n.id_status_fk, n.dt_cadastro, s.status');
        $this->db->from('tb_noticia n');
        $this->db->join('tb_status s', 's.id_status_pk = n.id_status_fk', 'inner');
        if (!empty($filter)):
            $where = implode(" OR ", $filter);
            $this->db->where($where);
        endif;
        $this->db->order_by($this->orderBy, $this->orderType);
        $this->db->limit($this->length, $this->start);
        $query_dados = $this->db->get();
        $resp_dados  = $query_dados->result();

        # Criar classe predefinida
        $notics = array();
        if (!empty($resp_dados)):

            foreach ($resp_dados as $value):
                # Botao
                $id_noticia = $value->id_noticia_pk;
                $url_edit   = base_url('./admin/noticia/editar/'.$id_noticia);
                $url_view   = base_url('./admin/noticia/ver/'.$id_noticia);
                $acao       = "<button type='button' class='btn btn-success btn-xs btn-acao' title='Editar Not&iacute;cia' onclick='Noticia.redirect(\"$url_edit\")'><i class='glyphicon glyphicon-edit' aria-hidden='true'></i></button>";
                $acao      .= "<button type='button' class='btn btn-warning btn-xs btn-acao' title='Visualizar Not&iacute;cia' onclick='Noticia.redirect(\"$url_view\")'><i class='glyphicon glyphicon-eye-open' aria-hidden='true'></i></button>";
                $acao      .= "<button type='button' class='btn btn-danger btn-xs btn-acao' title='Excluir Not&iacute;cia' onclick='Noticia.del(\"$id_noticia\")'><i class='glyphicon glyphicon-remove' aria-hidden='true'></i></button>";

                # Data
                $dt_cadastro = date("d/m/Y", strtotime($value->dt_cadastro));

                $notic              = new stdClass();
                $notic->titulo      = $value->titulo;
                $notic->dt_cadastro = $dt_cadastro;
                $notic->status      = $value->status;
                $notic->acao        = $acao;
                $notics[]           = $notic;
            endforeach;

        endif;

        $dados['draw']            = intval($this->draw);
        $dados['recordsTotal']    = $this->recordsTotal;
        $dados['recordsFiltered'] = $this->recordsTotal;
        $dados['data']            = $notics;

        return $dados;
    }

    /**
     * Método de exclusão de um Noticia
     *
     * @method delNoticia
     * @access public
     * @param integer $id Id do registro a ser excluído
     * @return obj Status da ação
     */
    public function delNoticia($id)
    {
        # Atribuir vars
        $retorno = new stdClass();
        $log     = array();

        # SQL
        $this->db->where('id_noticia_pk', $id);
        $this->db->delete('tb_noticia');

        if ($this->db->affected_rows() > 0) {
            $retorno->status = TRUE;
            $retorno->msg    = "Exclus&atilde;o realizada com Sucesso!";
            # Gravar Log
            $log['id_acao_pk']    = 3;
            $log['id_usuario_fk'] = $this->session->userdata('id_user');
            $log['id_noticia_fk'] = $id;
            $log['dt_hr_cad']     = date("Y-m-d H:i");
            $this->db->insert('tb_noticia_log', $log);
        } else {
            $retorno->status = FALSE;
            $retorno->msg    = "Houve um erro ao Excluir! Tente novamente...";
        }

        # retornar
        return $retorno;
    }
}

/* End of file noticia_model.php */
/* Location: ./application/models/noticia_model.php */