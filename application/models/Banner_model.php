<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Banner_model extends CI_Model {

    # Propriedades
    public $draw;
    public $orderBy;
    public $orderType;
    public $start;
    public $length;
    public $filter;
    public $columns;
    public $recordsTotal;
    public $recordsFiltered;

    public function __construct()
    {
        parent::__construct();
    }

    /**
     * Método responsável por cadastrar / editar um banner
     *
     * @method setBanner
     * @param obj $valores Dados para cadastro / edicao
     * @access public
     * @return obj Status de ação
     */
    public function setBanner($valores)
    {
        # Atribuir vars
        $retorno = new stdClass();
        $dados   = array();
        $log     = array();
        $data    = date("Y-m-d H:i");

        $dados['img']          = $valores->img;
        $dados['title']        = $valores->titulo;
        $dados['subtitle']     = $valores->subtitulo;
        $dados['description']  = $valores->descricao;
        $dados['btn_text']     = $valores->btn_text;
        $dados['btn_link']     = $valores->btn_link;
        $dados['pos_elem']     = $valores->pos_elem;
        $dados['id_status_fk'] = $valores->status;

        if (isset($valores->id) && $valores->id != ""):
            # Atualiza banner
            $this->db->where('id_banner_pk', $valores->id);
            $this->db->update('tb_banner', $dados);

            if ($this->db->affected_rows() >= 0) {
                $retorno->status = TRUE;
                $retorno->msg    = "Edi&ccedil;&atilde;o realizada com Sucesso!";
                # Gravar Log
                $log['id_acao_pk']    = 2;
                $log['id_usuario_fk'] = $this->session->userdata('id_user');
                $log['id_banner_fk']  = $valores->id;
                $log['dt_hr_cad']     = $data;
                $this->db->insert('tb_banner_log', $log);
            } else {
                $retorno->status = FALSE;
                $retorno->msg    = "Houve um erro ao editar! Tente novamente...";
            }
        else:
            # Grava banner
            $this->db->insert('tb_banner', $dados);

            if ($this->db->affected_rows() > 0) {
                $retorno->status = TRUE;
                $retorno->msg    = "Cadastro realizado com Sucesso!";
                # Gravar Log
                $log['id_acao_pk']    = 1;
                $log['id_usuario_fk'] = $this->session->userdata('id_user');
                $log['id_banner_fk']  = $this->db->insert_id();
                $log['dt_hr_cad']     = $data;
                $this->db->insert('tb_banner_log', $log);
            } else {
                $retorno->status = FALSE;
                $retorno->msg    = "Houve um erro ao cadastrar! Tente novamente...";
            }
        endif;

        # retornar
        return $retorno;
    }

    /**
     * Método responsável por pesquisar e buscar banners
     *
     * @method getBanners
     * @param obj $search Conjuntos de dados para realizar a pesquisa
     * @access public
     * @return obj Lista de banners
     */
    public function getBanners($search)
    {
        # Atribuir valores
        $this->draw      = $search->draw;
        $this->orderBy   = $search->orderBy;
        $this->orderType = $search->orderType;
        $this->start     = $search->start;
        $this->length    = $search->length;
        $this->filter    = $search->filter;
        $this->columns   = $search->columns;
        $filter          = array();
        $where           = array();

        # Se houver busca pela grid
        if ($this->filter != NULL):
            for($i=0; $i<count($this->columns); $i++):
                if ($this->columns[$i]['searchable'] === "true"):
                    $column = $this->columns[$i]['data'];
                    $filter[]= "$column LIKE '%{$this->filter}%'";
                endif;
            endfor;
        endif;

        # Contar total de registros
        $this->db->select('COUNT(b.id_banner_pk) AS total');
        $this->db->from('tb_banner b');
        $this->db->join('tb_status s', 's.id_status_pk = b.id_status_fk', 'inner');
        if (!empty($filter)):
            $where = implode(" OR ", $filter);
            $this->db->where($where);
        endif;
        $query            = $this->db->get();
        $respRecordsTotal = $query->result();
        if (!empty($respRecordsTotal)):
            $this->recordsTotal = $respRecordsTotal[0]->total;
        else:
            $this->recordsTotal = 0;
        endif;

        # Consultar banners
        $this->db->select('b.id_banner_pk, b.title, s.status');
        $this->db->from('tb_banner b');
        $this->db->join('tb_status s', 's.id_status_pk = b.id_status_fk', 'inner');
        if (!empty($filter)):
            $where = implode(" OR ", $filter);
            $this->db->where($where);
        endif;
        $this->db->order_by($this->orderBy, $this->orderType);
        $this->db->limit($this->length, $this->start);
        $query_dados = $this->db->get();
        $resp_dados  = $query_dados->result();

        # Criar classe predefinida
        $bans = array();
        if (!empty($resp_dados)):

            foreach ($resp_dados as $value):
                # Botao
                $id_banner = $value->id_banner_pk;
                $url_edit    = base_url('./admin/banner/editar/'.$id_banner);
                $url_view    = base_url('./admin/banner/ver/'.$id_banner);
                $acao        = "<button type='button' class='btn btn-success btn-xs btn-acao' title='Editar Banner' onclick='Banner.redirect(\"$url_edit\")'><i class='glyphicon glyphicon-edit' aria-hidden='true'></i></button>";
                $acao       .= "<button type='button' class='btn btn-warning btn-xs btn-acao' title='Visualizar Banner' onclick='Banner.redirect(\"$url_view\")'><i class='glyphicon glyphicon-eye-open' aria-hidden='true'></i></button>";
                $acao       .= "<button type='button' class='btn btn-danger btn-xs btn-acao' title='Excluir Banner' onclick='Banner.del(\"$id_banner\")'><i class='glyphicon glyphicon-remove' aria-hidden='true'></i></button>";

                $ban         = new stdClass();
                $ban->title  = $value->title;
                $ban->status = $value->status;
                $ban->acao   = $acao;
                $bans[]      = $ban;
            endforeach;

        endif;

        $dados['draw']            = intval($this->draw);
        $dados['recordsTotal']    = $this->recordsTotal;
        $dados['recordsFiltered'] = $this->recordsTotal;
        $dados['data']            = $bans;

        return $dados;
    }

    /**
     * Método de exclusão de um Banner
     *
     * @method delBanner
     * @access public
     * @param integer $id Id do registro a ser excluído
     * @return obj Status da ação
     */
    public function delBanner($id)
    {
        # Atribuir vars
        $retorno = new stdClass();
        $log     = array();

        # SQL
        $this->db->where('id_banner_pk', $id);
        $this->db->delete('tb_banner');

        if ($this->db->affected_rows() > 0) {
            $retorno->status = TRUE;
            $retorno->msg    = "Exclus&atilde;o realizada com Sucesso!";
            # Gravar Log
            $log['id_acao_pk']    = 3;
            $log['id_usuario_fk'] = $this->session->userdata('id_user');
            $log['id_banner_fk']  = $id;
            $log['dt_hr_cad']     = date("Y-m-d H:i");
            $this->db->insert('tb_banner_log', $log);
        } else {
            $retorno->status = FALSE;
            $retorno->msg    = "Houve um erro ao Excluir! Tente novamente...";
        }

        # retornar
        return $retorno;
    }
}

/* End of file banner_model.php */
/* Location: ./application/models/banner_model.php */